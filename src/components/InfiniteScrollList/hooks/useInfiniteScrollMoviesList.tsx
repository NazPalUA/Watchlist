...
export default function useInfiniteScrollMoviesList(input: SearchTypes | PopularTypes) {
    ...
    const { data, isLoading, error } = useFetch<PopularOrSearchMoviesAPIResponse>(url)

    // Callback to update movies state and handle pagination
    const updateMovies = useCallback((newData: PopularOrSearchMoviesAPIResponse) => {
        setMovies(prevMovies => {
            // Combine new data with existing movies
            const combinedData = page === 1 ? newData.results : [...prevMovies, ...newData.results]
